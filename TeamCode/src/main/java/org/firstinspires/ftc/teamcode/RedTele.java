...
public class UltimateTeleOpRed extends OpMode {
    ...
    private Pose getStartPoseFromAuto() {
        // Try to get pose from ball12blue (mirrored for red)
        try {
            ...
            if (bluePose != null && (bluePose.getX() != 0 || bluePose.getY() != 0)) {
                return bluePose.mirror(); // Mirror blue to red
            }
        } catch (Exception e) {
            telemetry.addLine("No auto end pose found, using default");
        }

        // Default red start position
        return new Pose(48.9, 66.6, Math.toRadians(180)); // Mirrored from blue
    }
}