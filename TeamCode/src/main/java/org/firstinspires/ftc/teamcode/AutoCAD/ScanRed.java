...
@Autonomous(name = "Obelisk Scan Auto - RED", group = "Auto")
public class ScanRed extends OpMode {
    ...
    // Waypoints (RED side - mirrored from BLUE)
    private static final Pose startPos = new Pose(56.000, 8.000, Math.toRadians(90)).mirror();
    private static final Pose scanPos = new Pose(57.821, 34.841, Math.toRadians(90)).mirror();
    private static final Pose shootPos = new Pose(56.890, 12.214, Math.toRadians(110)).mirror();

    // Intake positions for each tag (RED side - mirrored)
    private static final Pose intakeID21 = new Pose(11.014, 36.110, Math.toRadians(0)).mirror();
    private static final Pose intakeID22 = new Pose(18.131, 58.455, Math.toRadians(0)).mirror();
    private static final Pose intakeID23 = new Pose(16.972, 83.448, Math.toRadians(0)).mirror();

    // Control points for curved path (RED side - mirrored)
    private static final Pose curveControl1 = new Pose(63.624, 63.703, Math.toRadians(0)).mirror();
    private static final Pose curveControl2 = new Pose(45.272, 58.421, Math.toRadians(0)).mirror();
    ...
    public void init() {
        ...
        telemetry.addData("Status", "Initialized - Obelisk Scan Auto RED");
    ...
    private void buildShootPath(Pose fromPose) {
        ShootPosition = follower.pathBuilder()
                ...
                .setLinearHeadingInterpolation(fromPose.getHeading(), shootPos.getHeading())
                .build();
    }
