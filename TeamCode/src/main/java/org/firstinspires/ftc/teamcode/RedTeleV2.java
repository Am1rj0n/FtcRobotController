...
public class RedTeleV2 extends OpMode {
    ...
    public void init() {
        ...
        drivetrain = new DrivetrainSubsystem(hardwareMap, follower, IS_RED, limelight);
    ...
    private Pose getStartPoseFromAuto() {
        try {
            Pose redPose = org.firstinspires.ftc.teamcode.paths.robotv2.Ball12RedFar.autoEndPose;
            if (redPose != null && (redPose.getX() != 0 || redPose.getY() != 0)) {
                return redPose;
            }
        } catch (Exception e) {
            // No auto end pose found
        }
        return new Pose(50.393, 22.331, Math.toRadians(65.5));
    }
}