...
@Autonomous(name = "Scan Auto Blue - V3", group = "Autonomous")
@Configurable
public class ScanBlue extends OpMode {
    ...
    private static final Pose shootPos = new Pose(58.579, 13.828, Math.toRadians(113.5));
            ...
            case SPIN_UP:
                ...
                if (shootTimer.seconds() > SPIN_UP_DURATION) {
                    ...
                    PathChain toScan = follower.pathBuilder()
                            ...
                            .setLinearHeadingInterpolation(startPos.getHeading(), scanPos.getHeading())
                            .build();
            ...
            case SCANNING:
                ...
                if ((detectedTagId >= 21 && detectedTagId <= 23) || scanTimer.seconds() > SCAN_TIMEOUT) {
                    ...
                    PathChain toShoot = follower.pathBuilder()
                            .addPath(new BezierLine(scanPos, shootPos))
                            .setLinearHeadingInterpolation(scanPos.getHeading(), shootPos.getHeading())
                            .build();
    ...
    private void buildDynamicPaths(int tagId) {
        Pose intakePrePos, intakePreAdjust, intakeFinal, intakeAdjust, shootReturn;

        if (tagId == 21) {
            // ID 21 paths
            intakePrePos = new Pose(58.579, 13.828);
            intakePreAdjust = new Pose(56.662, 31.166);
            intakeAdjust = new Pose(56.662, 33.166);
            intakeFinal = new Pose(20, 33.441);
            shootReturn = new Pose(58.579, 13.828);

            IntakePathChain = follower.pathBuilder()
                    .addPath(new BezierLine(intakePrePos, intakePreAdjust))
                    .setLinearHeadingInterpolation(Math.toRadians(113.5), Math.toRadians(0))
                    .addPath(new BezierLine(intakeAdjust, intakeFinal))
                    .setTangentHeadingInterpolation()
                    .setReversed()
                    .build();

            ReturnShootPath = follower.pathBuilder()
                    .addPath(new BezierLine(intakeFinal, shootReturn))
                    .setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(113.5))
                    .build();

        } else if (tagId == 22) {
            // ID 22 paths
            intakePrePos = new Pose(58.579, 13.828);
            intakePreAdjust = new Pose(58.386, 50.931);
            intakeAdjust = new Pose(29.186, 50.952);
            intakeFinal = new Pose(29.186, 50.952);
            shootReturn = new Pose(58.607, 13.290);

            IntakePathChain = follower.pathBuilder()
                    .addPath(new BezierLine(intakePrePos, intakePreAdjust))
                    .setLinearHeadingInterpolation(Math.toRadians(113.5), Math.toRadians(0))
                    .addPath(new BezierLine(intakePreAdjust, intakeFinal))
                    .setTangentHeadingInterpolation()
                    .setReversed()
                    .build();

            Pose adjustedReturn = new Pose(29.186, 59.952);
            ReturnShootPath = follower.pathBuilder()
                    .addPath(new BezierLine(adjustedReturn, shootReturn))
                    .setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(113.5))
                    .build();

        } else { // tagId == 23
            // ID 23 paths
            intakePrePos = new Pose(58.607, 13.290);
            intakePreAdjust = new Pose(58.262, 78.248);
            intakeAdjust = new Pose(26, 78.145);
            intakeFinal = new Pose(26, 78.145);
            shootReturn = new Pose(58.841, 13.221);
            ...
            Pose adjustedReturn = new Pose(26, 80.145);
            ReturnShootPath = follower.pathBuilder()
                    .addPath(new BezierLine(adjustedReturn, shootReturn))
